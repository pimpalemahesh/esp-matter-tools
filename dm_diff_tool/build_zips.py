...
def main() -> int:
    ...
    if args.check:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # --- Check for orphaned zips (committed but no longer in upstream) ---
            for zip_file in sorted(committed_zips_dir.glob("*.zip")):
                ver = zip_file.stem
                if ver not in new_manifest:
                    print(f"{ver}.zip is orphaned (no longer in upstream data model)")
                    drift = True
    ...
    # Remove zips for versions that no longer exist in the upstream data model.
    for zip_file in sorted(zips_dir.glob("*.zip")):
        if zip_file.stem not in manifest:
            zip_file.unlink()
            print(f"  Removed orphaned {zip_file.name}")
