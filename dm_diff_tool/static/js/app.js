...
const xmlMapCache = {}; // "version:category" -> { filename: xml_string }
...
async function loadManifest(status) {
  ...
  if (!resp.ok) {
    throw new Error(
      `Cannot load data_manifest.json (HTTP ${
        resp.status
      }). Run ./build_tool.sh before serving.`,
    );
  }
...
async function init() {
  ...
  // Load diff engine (fetch in parallel, execute in order so imports resolve)
  status.textContent = "Loading diff engine...";
  const [parserCode, differCode, engineCode] = await Promise.all([
    fetch(BASE + "dm_parser.py").then((r) => r.text()),
    fetch(BASE + "dm_differ.py").then((r) => r.text()),
    fetch(BASE + "diff_engine.py").then((r) => r.text()),
  ]);
  pyodide.runPython(parserCode);
  pyodide.runPython(differCode);
...
async function runDiff() {
  ...
  document.getElementById("resultArea").innerHTML = `
    <div class="loading"><div class="spinner"></div><div>Computing diff between ${esc(
      oldVer,
    )} and ${esc(newVer)}...</div></div>`;
...
function stripInternal(obj) {
  ...
  if (obj && typeof obj === "object") {
    ...
    for (const [k, v] of Object.entries(obj)) {
      if (!k.startsWith("_")) out[k] = stripInternal(v);
    }
    return out;
  }
  return obj;
...
function renderDiff(oldVer, newVer) {
  ...
  if (nameFilter && isFocused) {
    html += `<div class="search-info focused"><span class="info-icon">&#8942;</span>
      Showing element-level diff for <strong>"${esc(
        nameFilter,
      )}"</strong> within matching clusters / device types</div>`;
  } else if (nameFilter && isBroad) {
    html += `<div class="search-info broad"><span class="info-icon">&#9432;</span>
      No exact element match for <strong>"${esc(
        nameFilter,
      )}"</strong>. Showing related clusters / device types containing the term</div>`;
  }
  ...
  cats.forEach(([key, label], i) => {
    ...
    html += renderCategoryDiff(currentDiff[key], key);
    html += `</div>`;
  });
...
function updateSummary() {
  ...
  bar.innerHTML = `
    <div class="summary-card added"><div class="count">${
      nAdded
    }</div><div class="label">${label} Added</div></div>
    <div class="summary-card removed"><div class="count">${
      nRemoved
    }</div><div class="label">${label} Removed</div></div>
    <div class="summary-card modified"><div class="count">${
      nModified
    }</div><div class="label">${
      label
    } Modified</div><div style="font-size:11px;color:var(--text-dim);margin-top:2px">${
      nRevUp
    } rev updated &middot; ${nNoRev} no rev update</div></div>
    <div class="summary-card unchanged"><div class="count">${
      nUnchanged
    }</div><div class="label">${label} Unchanged</div></div>`;
}
...
function renderCategoryDiff(diff, catKey) {
  if (!diff) return "";
  ...
  if (Object.keys(added).length) {
    html += `<div class="diff-section"><div class="diff-group-title"><span class="dot added-dot"></span> Added ${
      label
    }s (${Object.keys(added).length})</div>`;
  ...
  if (Object.keys(removed).length) {
    html += `<div class="diff-section"><div class="diff-group-title"><span class="dot removed-dot"></span> Removed ${
      label
    }s (${Object.keys(removed).length})</div>`;
  ...
  if (Object.keys(modified).length) {
    ...
    if (Object.keys(revUpdated).length) {
      html += `<div class="diff-section"><div class="diff-group-title"><span class="dot modified-dot"></span> Modified ${
        label
      }s — Revision Updated (${Object.keys(revUpdated).length})</div>`;
      for (const [file, data] of Object.entries(revUpdated)) {
        html += renderModifiedCard(file, data, isCluster);
      }
      html += `</div>`;
    }

    if (Object.keys(noRevUpdate).length) {
      html += `<div class="diff-section"><div class="diff-group-title"><span class="dot norev-dot"></span> Modified ${
        label
      }s — No Revision Update (${Object.keys(noRevUpdate).length})</div>`;
      for (const [file, data] of Object.entries(noRevUpdate)) {
        html += renderModifiedCard(file, data, isCluster, true);
      }
      html += `</div>`;
    }
  }
  ...
  if (unchanged.length) {
    html += `<div class="diff-section"><div class="diff-group-title" style="color:var(--unchanged-text)">Unchanged ${
      label
    }s (${unchanged.length})</div>`;
    html += `<div style="color:var(--unchanged-text);font-size:13px;padding:4px 0;">${unchanged
      .map((f) => esc(f.replace(".xml", "")))
      .join(", ")}</div>`;
    html += `</div>`;
  }
...
function renderAddedCard(file, data, isCluster) {
  ...
  let html = `<div class="diff-card added-card" id="${id}">
    <div class="diff-card-header" onclick="toggleCard('${id}')">
      <div><span class="name">${esc(
        data.name,
      )}</span> <span style="color:var(--text-dim);font-size:12px">${esc(
        file,
      )}</span></div>
      <div class="meta">
...
function renderModifiedCard(file, data, isCluster, noRev) {
  const id = "card-" + file.replace(/[^a-zA-Z0-9]/g, "_");
  const changes = data.changes || {};
  const changeSections = Object.keys(changes).filter(
    (k) => k !== "revision" && k !== "revisions",
  );
  ...
  if (changes.revision) {
    html += `<div class="kv-change"><span class="kv-key">Revision:</span>
      ...
      <span class="val-new">${esc(
        String(changes.revision.new || ""),
      )}</span></div>`;
  }
  ...
  if (changes.features) {
    html += renderSectionDiff("Features", changes.features, [
      "bit",
      "code",
      "name",
      "summary",
  ...
  if (changes.commands) {
    html += renderSectionDiff("Commands", changes.commands, [
      "id",
      "name",
      "direction",
      "response",
  ...
  if (changes.events) {
    html += renderSectionDiff("Events", changes.events, [
      "id",
      "name",
      "priority",
      "conformance",
...
function renderRevisionsDiff(revDiff) {
  ...
  if (revDiff.added && revDiff.added.length) {
    ...
    for (const r of revDiff.added) {
      html += `<tr class="row-added"><td>${esc(r.revision)}</td><td>${esc(
        r.summary,
      )}</td><td><span class="badge-sm badge-added">NEW</span></td></tr>`;
    }
    html += `</tbody></table>`;
  }
  if (revDiff.modified && revDiff.modified.length) {
    ...
    for (const m of revDiff.modified) {
      for (const c of m.changes) {
        html += `<tr class="row-modified"><td>${esc(m.key)}</td><td>${esc(
          c.field,
        )}</td>
          <td><span class="val-old">${esc(
            String(c.old),
          )}</span><span class="arrow">&rarr;</span><span class="val-new">${esc(
            String(c.new),
          )}</span></td></tr>`;
      }
    }
...
function renderSectionDiff(title, diff, columns) {
  ...
  for (const [key, mdata] of Object.entries(modified)) {
    ...
    for (const col of columns) {
      if (changeMap[col]) {
        html += `<td><span class="val-old">${esc(
          formatVal(changeMap[col].old),
        )}</span>
          <span class="arrow">&rarr;</span>
          <span class="val-new">${esc(
            formatVal(changeMap[col].new),
          )}</span></td>`;
      } else {
...
function renderNestedFieldsDiff(parentKey, fieldsDiff, colspan) {
  ...
  if (fieldsDiff.added && fieldsDiff.added.length) {
    for (const f of fieldsDiff.added) {
      ...
      html += fieldCols
        .map((c) => `<strong>${c}:</strong> ${esc(formatVal(f[c]))}`)
        .join(" &middot; ");
      html += `</td></tr>`;
    }
  }
  ...
  if (fieldsDiff.modified && fieldsDiff.modified.length) {
    for (const m of fieldsDiff.modified) {
      ...
      html += `<span class="badge-sm badge-modified" style="margin-right:8px">~ FIELD ${esc(
        m.key,
      )}</span> `;
      for (const c of m.changes) {
        html += `<strong>${esc(c.field)}:</strong> <span class="val-old">${esc(
          String(c.old),
        )}</span><span class="arrow">&rarr;</span><span class="val-new">${esc(
          String(c.new),
        )}</span> &middot; `;
...
function renderDataTypesDiff(diff) {
  ...
  for (const [key, item] of Object.entries(added)) {
    ...
    html += `<span class="badge-sm badge-added">ADDED</span> <strong>${esc(
      item.kind,
    )}: ${esc(item.name)}</strong>`;
  ...
  for (const [key, item] of Object.entries(removed)) {
    ...
    html += `<span class="badge-sm badge-removed">REMOVED</span> <strong>${esc(
      item.kind,
    )}: ${esc(item.name)}</strong>`;
  ...
  if (items.length) {
    ...
    for (const it of items) {
      ...
      for (const k of Object.keys(it)) {
        html += `<td><span style="color:var(--text-dim);font-size:11px">${esc(
          k,
        )}:</span> ${esc(formatVal(it[k]))}</td>`;
      }
      html += `</tr>`;
    }
    html += `</tbody></table>`;
...
function renderDataTypeItemsDiff(diff) {
  ...
  if (diff.added && diff.added.length) {
    ...
    for (const item of diff.added) {
      ...
      html += Object.entries(item)
        .map(([k, v]) => `<strong>${esc(k)}</strong>=${esc(formatVal(v))}`)
        .join(" ");
      html += `</div>`;
    }
    html += `</div>`;
  ...
  if (diff.modified && diff.modified.length) {
    ...
    for (const m of diff.modified) {
      html += `<div style="padding:2px 0"><span class="badge-sm badge-modified">~</span> <strong>${esc(
        m.key,
      )}</strong>: `;
      for (const c of m.changes) {
        html += `${esc(c.field)}: <span class="val-old">${esc(
          String(c.old),
        )}</span><span class="arrow">&rarr;</span><span class="val-new">${esc(
          String(c.new),
        )}</span> `;
...
function renderDeviceTypeClustersDiff(diff) {
  ...
  if (Object.keys(added).length) {
    ...
    for (const [key, item] of Object.entries(added)) {
      html += `<tr class="row-added">
        <td>${esc(item.id)}</td><td>${esc(item.name)}</td><td>${esc(
          item.side,
        )}</td>
        <td>${esc(
          item.conformance,
        )}</td><td><span class="badge-sm badge-added">ADDED</span></td></tr>`;
      if (item.features && Object.keys(item.features).length) {
        for (const [fc, fv] of Object.entries(item.features)) {
          html += `<tr class="row-added"><td colspan="5" style="padding-left:32px">Feature: <strong>${esc(
            fc,
          )}</strong> (${esc(fv.conformance)})</td></tr>`;
        }
      }
      if (item.commands && Object.keys(item.commands).length) {
        for (const [ci, cv] of Object.entries(item.commands)) {
          html += `<tr class="row-added"><td colspan="5" style="padding-left:32px">Command: <strong>${esc(
            cv.name,
          )}</strong> [${esc(ci)}] (${esc(cv.conformance)})</td></tr>`;
        }
      }
    }
    html += `</tbody></table>`;
  ...
  if (Object.keys(removed).length) {
    ...
    for (const [key, item] of Object.entries(removed)) {
      html += `<tr class="row-removed">
        ...
        <td>${esc(
          item.conformance,
        )}</td><td><span class="badge-sm badge-removed">REMOVED</span></td></tr>`;
    }
    html += `</tbody></table>`;
  }
  ...
  if (Object.keys(modified).length) {
    for (const [key, mdata] of Object.entries(modified)) {
      ...
      const displayName = newItem.name
        ? `${esc(newItem.name)} [${esc(newItem.id || key)}] (${esc(
            newItem.side || "",
          )})`
        : esc(key);
      ...
      for (const [field, change] of Object.entries(changes || {})) {
        if (
          ...
          html += `<div style="margin-top:4px"><strong>${esc(
            field,
          )}:</strong></div>`;
            ...
            for (const [ak, av] of Object.entries(change.added)) {
              ...
              if (typeof av === "object") {
                html += Object.entries(av)
                  .map(
                    ([k2, v2]) =>
                      `<strong>${esc(k2)}</strong>=${esc(formatVal(v2))}`,
                  )
                  .join(" ");
              } else {
          ...
          if (change.modified && Object.keys(change.modified).length) {
            for (const [mk, mv] of Object.entries(change.modified)) {
              html += `<div style="padding:2px 0 2px 16px"><span class="badge-sm badge-modified">~</span> <strong>${esc(
                mk,
              )}</strong>: `;
              if (mv && typeof mv === "object") {
                for (const [f2, c2] of Object.entries(mv)) {
                  if (c2 && typeof c2 === "object" && "old" in c2) {
                    html += `${esc(f2)}: <span class="val-old">${esc(
                      formatVal(c2.old),
                    )}</span><span class="arrow">&rarr;</span><span class="val-new">${esc(
                      formatVal(c2.new),
                    )}</span> `;
  ...
  for (const [key, item] of Object.entries(added)) {
    ...
    if (item.requirements) {
      for (const [rk, rv] of Object.entries(item.requirements)) {
        html += `<div style="padding:2px 0 2px 16px">${esc(rv.name)}: <em>${esc(
          rv.conformance,
        )}</em></div>`;
      }
    }
  ...
  for (const [key, mdata] of Object.entries(modified)) {
    ...
    for (const [field, change] of Object.entries(changes || {})) {
      if (
        ...
        if (change.added && Object.keys(change.added).length) {
          for (const [ak, av] of Object.entries(change.added)) {
            html += `<div style="padding:2px 0 2px 16px"><span class="badge-sm badge-added">+</span> ${esc(
              av.name,
            )}: <em>${esc(av.conformance)}</em></div>`;
          }
        }
        if (change.removed && Object.keys(change.removed).length) {
          for (const [rk, rv] of Object.entries(change.removed)) {
            html += `<div style="padding:2px 0 2px 16px"><span class="badge-sm badge-removed">-</span> ${esc(
              rv.name,
            )}: <em>${esc(rv.conformance)}</em></div>`;
          }
        }
        if (change.modified && Object.keys(change.modified).length) {
          for (const [mk, mv] of Object.entries(change.modified)) {
            html += `<div style="padding:2px 0 2px 16px"><span class="badge-sm badge-modified">~</span> ${esc(
              mk,
            )}: `;
            if (mv && typeof mv === "object" && "old" in mv) {
              html += `<span class="val-old">${esc(
                formatVal(mv.old),
              )}</span><span class="arrow">&rarr;</span><span class="val-new">${esc(
                formatVal(mv.new),
              )}</span>`;
...
function renderFullItem(data, isCluster, type) {
  ...
  html += `<div class="kv-change"><span class="kv-key">ID:</span> ${esc(
    data.id,
  )}</div>`;
  html += `<div class="kv-change"><span class="kv-key">Revision:</span> ${esc(
    data.revision,
  )}</div>`;
  ...
  if (isCluster) {
    if (data.features && Object.keys(data.features).length) {
      ...
      for (const [k, f] of Object.entries(data.features)) {
        html += `<tr class="${cls}"><td>${esc(f.bit)}</td><td>${esc(
          f.code,
        )}</td><td>${esc(f.name)}</td><td>${esc(
          f.summary,
        )}</td><td>${esc(f.conformance)}</td></tr>`;
      }
      html += `</tbody></table></div>`;
    }
    ...
    if (data.attributes && Object.keys(data.attributes).length) {
      ...
      for (const [k, a] of Object.entries(data.attributes)) {
        html += `<tr class="${cls}"><td>${esc(a.id)}</td><td>${esc(
          a.name,
        )}</td><td>${esc(a.type)}</td><td>${esc(
          a.conformance,
        )}</td><td>${esc(a.access)}</td></tr>`;
      }
      html += `</tbody></table></div>`;
    }
    ...
    if (data.commands && Object.keys(data.commands).length) {
      ...
      for (const [k, c] of Object.entries(data.commands)) {
        html += `<tr class="${cls}"><td>${esc(c.id)}</td><td>${esc(
          c.name,
        )}</td><td>${esc(c.direction)}</td><td>${esc(
          c.response,
        )}</td><td>${esc(c.conformance)}</td></tr>`;
      }
      html += `</tbody></table></div>`;
    }
    ...
    if (data.events && Object.keys(data.events).length) {
      ...
      for (const [k, e] of Object.entries(data.events)) {
        html += `<tr class="${cls}"><td>${esc(e.id)}</td><td>${esc(
          e.name,
        )}</td><td>${esc(e.priority)}</td><td>${esc(e.conformance)}</td></tr>`;
      }
      html += `</tbody></table></div>`;
    }
  } else {
    if (
      data.conditionRequirements &&
      ...
      for (const [k, cr] of Object.entries(data.conditionRequirements)) {
        for (const [rk, rv] of Object.entries(cr.requirements || {})) {
          html += `<tr class="${cls}"><td>${esc(cr.name)} [${esc(cr.id)}]</td><td>${esc(
            rv.name,
          )}</td><td>${esc(rv.conformance)}</td></tr>`;
        }
      }
      html += `</tbody></table></div>`;
    }
    ...
    if (data.clusters && Object.keys(data.clusters).length) {
      ...
      for (const [k, c] of Object.entries(data.clusters)) {
        html += `<tr class="${cls}"><td>${esc(c.id)}</td><td>${esc(
          c.name,
        )}</td><td>${esc(c.side)}</td><td>${esc(c.conformance)}</td></tr>`;
      }
      html += `</tbody></table></div>`;
    }
  }
