    ...
    try:
        ...
        if isinstance(actual_feature_map, str):
            ...
            return False, [
                {
                    "type": "feature",
                    "message": f"Invalid feature_map format in cluster {cluster_id}",
                }
            ]
                ...
                missing_features.append(
                    {
                        ...
                        "feature_bitmask": f"0x{feature_bitmask:X}",
                        "feature_map_value": f"0x{feature_map_value:X}",
                        "check_result": f"0x{feature_map_value:X} & 0x{feature_bitmask:X} = 0x{feature_map_value & feature_bitmask:X}",
                    }
                )
...
def validate_revisions(actual_revision, required_revision, item_type, item_id, item_name):
    ...
    try:
        ...
        return False, [
            {
                "type": "revision_error",
                "message": f"Revision validation error for {item_type} {item_name}: {str(e)}",
            }
        ]
...
def load_element_requirements(chip_version):
    ...
    Args:
        chip_version (str): Version of the chip requirements to load

    Returns:
        list: List of device type requirements, empty list if file not found
    """
    try:
...
def validate_cluster(endpoint_clusters, required_cluster):
    ...
    Args:
        endpoint_clusters (dict): Dictionary of all clusters in the endpoint
        required_cluster (dict): Required cluster configuration
        ...
        if required_features:
            ...
            if isinstance(feature_map_data, dict) and "FeatureMap" in feature_map_data:
                ...
                result["missing_elements"].append(
                    {
                        "type": "feature",
                        "message": f"FeatureMap not found in cluster {cluster_name}, but features are required",
                        "cluster_id": cluster_id,
                        "cluster_name": cluster_name,
                    }
                )
...
def validate_single_device_type(endpoint, device_type_id, device_requirements):
    ...
    Args:
        endpoint (dict): The endpoint data containing clusters
        device_type_id (int): The device type ID to validate
        device_requirements (dict): The requirements for this device type
    ...
    for required_cluster in required_clusters:
        ...
        try:
            ...
            logger.error(f"Error validating cluster {cluster_name}: {cluster_error}")
            result["cluster_validations"].append(
                {
                    "cluster_id": required_cluster.get("id", "unknown"),
                    "cluster_name": cluster_name,
                    "cluster_type": required_cluster.get("type", "server"),
                    "is_compliant": False,
                    "missing_elements": [
                        {
                            "type": "error",
                            "message": f"Cluster validation error: {str(cluster_error)}",
                        }
                    ],
                    "revision_issues": [],
                    "event_warnings": [],
...
def validate_device_compliance(parsed_data, element_requirements, chip_version):
    """Validate if the device meets all requirements for its device types.

    Args:
        parsed_data (dict): The parsed device data containing endpoints and clusters
        element_requirements (list): List of device type requirements
        chip_version (str): The chip version used for validation

    Returns:
        dict: Validation results with compliance status and missing elements
    """
                        ...
                        endpoint_result["device_types"].append(
                            {
                                ...
                                "error": f"No requirements found for device type 0x{device_type_id_int:04X}",
                            }
                        )
                        endpoint_result["is_compliant"] = False
