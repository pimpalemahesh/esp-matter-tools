...
def test_webapp_reports_knowledge_source():
    base = {"transport": ["wifi_2g"],
            "endpoints": [{"device_types": ["Extended Color Light"]}]}
    own = webapp.generate_scaffold_files({**base, "spec_version": "1.5.1"}, {"1": ["CC.S.F00"]})
    ...
    near = webapp.generate_scaffold_files({**base, "spec_version": "1.6"}, {"1": ["CC.S.F00"]})
    assert near["exact"] is True and "nearest" in near["knowledge_source"]  # 1.6 -> 1.5.1
