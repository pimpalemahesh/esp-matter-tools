...
def test_synthetic_fixpoint_and_leaves():
    ...
    role_profile = {"seeds": ["MCORE.ROLE.COMMISSIONEE"], "deny": ["MCORE.IDM.C", "MCORE.IDM.C.*"]}
    enabled = me._compute(items, _profile(transport=["wifi_2g"]), me.NodeFacts(), role_profile)
    ...
    assert "MCORE.DD.TXT_KEY_VP" not in enabled   # generic leaf: default OFF (no max-options)
...
def test_integration_base_xml_commissionee_wifi():
    ...
    enabled = me.compute_mcore_pics(_profile(transport=["wifi_2g"]), "1.6", cluster_ids=set())
    # Input-seeded and cond-derived stay on.
    for code in ["MCORE.COM.WIFI_2P4GHZ", "MCORE.COM.WIFI", "MCORE.COM.WIRELESS",
                 "MCORE.COM.BLE", "MCORE.ROLE.COMMISSIONEE", "MCORE.DD.QR",
    ...
    for code in ["MCORE.IDM.S", "MCORE.DD.TXT_KEY_VP", "MCORE.SC.SII_OP_DISCOVERY_KEY",
                 "MCORE.DD.PHYSICAL_TAMPERING", "MCORE.COM.THR", "MCORE.COM.ETH",
                 "MCORE.ROLE.COMMISSIONER", "MCORE.IDM.C", "MCORE.OTA.Requestor",
                 "MCORE.OTA.Provider", "MCORE.BRIDGE", "MCORE.DD.DISCOVERY_PAF",
...
def test_ota_and_bdx_derived_from_cluster_ids():
    def mcore(cluster_ids):
        return me.compute_mcore_pics(_profile(transport=["wifi_2g"]), "1.6", cluster_ids)
...
def test_onboarding_multiselect_enables_all():
    prof = DeviceProfile.from_dict(
        {"spec_version": "1.6", "device_type": "X", "transport": ["wifi_2g"],
...
def test_integration_ethernet_only_no_ble():
    ...
    assert "MCORE.COM.BLE" not in enabled  # ble_commissioning defaults false for ethernet-only
