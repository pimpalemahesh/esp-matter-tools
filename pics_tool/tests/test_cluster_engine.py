...
def test_feature_mask_fixpoint_chain():
    ...
    cluster = Cluster(
        id="0x1234", name="X", pics="X", revision=1,
        features={
            0: _feature(0, "A", cj({"type": "mandatory"})),
            1: _feature(1, "B", cj({"type": "mandatory",
                                    "condition": {"op": "feature", "code": "A", "bit": 0}})),
            2: _feature(2, "C", cj({"type": "optional"})),
        },
    )
    req = ClusterRequirement(id="0x1234", name="X", conformance=cj({"type": "mandatory"}))
...
def test_attribute_depends_on_feature():
    cluster = Cluster(
        ...
        attributes={
            ...
            "0x0001": Attribute("0x0001", "GatedOnA", cj(
                {"type": "mandatory", "condition": {"op": "feature", "code": "A", "bit": 0}})),
...
def test_transport_seed_forces_choice_feature():
    ...
    cluster = Cluster(
        id="0x0031", name="Network Commissioning", pics="CNET", revision=1,
        features={0: _feature(0, "WI", cj({"type": "optional",
                                           "choice": {"marker": "a", "more": False}}))},
    )
    req = ClusterRequirement(id="0x0031", name="Network Commissioning",
                             conformance=cj({"type": "mandatory"}))
    enabled = ce._enable_cluster(req, cluster, frozenset(), {"WI"})
    assert "CNET.S.F00" in enabled


def test_optional_cluster_excluded():
    req = ClusterRequirement(id="0x0008", name="Level Control",
                             conformance=cj({"type": "optional"}))
...
def test_onoff_light_wifi_expected_codes():
    ...
    ep1 = pics[1]
    for code in ["OO.S", "OO.S.A0000", "OO.S.A4000", "OO.S.C00.Rsp",
                 "OO.S.C01.Rsp", "OO.S.F00", "DESC.S"]:
    ...
    ep0 = pics[0]
    for code in ["BINFO.S", "ACL.S", "CGEN.S", "CNET.S", "CNET.S.F00",
                 "OPCREDS.S", "GRPKEY.S", "DGGEN.S"]:
