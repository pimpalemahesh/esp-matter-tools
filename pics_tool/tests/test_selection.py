...
def test_endpoint_string_and_device_type_forms():
    sel = Selection.from_dict({
        ...
        "endpoints": ["On/Off Light", {"device_type": "Dimmable Light", "claims": ["LVL.S.F01"]}],
    })
...
def test_errors():
    with pytest.raises(SelectionError):
        Selection.from_dict({"spec_version": "1.6", "transport": ["wifi_2g"]})  # no endpoints/device_type
    with pytest.raises(SelectionError):
        Selection.from_dict({"spec_version": "1.6", "transport": ["wifi_2g"], "endpoints": []})
    with pytest.raises(SelectionError):
        Selection.from_dict({"spec_version": "1.6", "transport": ["wifi_2g"],
                             "endpoints": [{"claims": ["OO.S.F02"]}]})  # endpoint w/o device_types
