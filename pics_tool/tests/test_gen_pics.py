...
def _supported(path: Path) -> set[str]:
    ...
    return {
        pi.find("itemNumber").text.strip()
        for pi in root.iter("picsItem")
        if (
            pi.find("support") is not None
            and (pi.find("support").text or "").strip() == "true"
        )
    }
...
def test_onoff_light_wifi_end_to_end(tmp_path):
    ...
    ep0_cnet = _supported(
        tmp_path / "endpoint0" / "Network Commissioning Cluster Test Plan.xml"
    )
...
def test_optional_cluster_not_emitted(tmp_path):
    ...
    endpoints = {
        ep.endpoint: set(ep.pics) for ep in generate_cluster_pics(model, profile)
    }
    write_pics("1.6", endpoints, tmp_path)
