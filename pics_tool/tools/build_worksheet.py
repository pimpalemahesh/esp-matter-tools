...
from pics_tool.generate.mcore_engine import (
    compute_mcore_pics,
    _TRANSPORT_SEEDS,
    _ROLE_SEEDS,
    _ONBOARDING_SEEDS,
...
def en(
    ...
    return compute_mcore_pics(DeviceProfile.from_dict(d), "1.6", set(clusters)) & all_items
...
DIMS = {
    ...
    "onboarding": [
        dict(onboarding=x) for x in ((), ("qr",), ("manual_pairing_code",), ("nfc",))
    ],
    "device_types": [
        dict(clusters=c)
        for c in (
...
manual = all_items - reach
seeds = set().union(
    *_TRANSPORT_SEEDS.values(),
    *_ROLE_SEEDS.values(),
    *_ONBOARDING_SEEDS.values(),
    {"MCORE.COM.BLE"},
)
comp = (
    {"MCORE.OTA.Requestor", "MCORE.OTA.Provider", "MCORE.BRIDGE"}
    | _BDX_REQUESTOR
    | _BDX_PROVIDER
)
ref = {}
...
def note(n, c, dims):
    ...
    if c == "server":
        return (
            "IM server — true for a device that hosts clusters"
            if n == "MCORE.IDM.S"
...
for n in order:
    ...
    ws.append(
        [
            ...
            ", ".join(dims),
            conds[n],
            str(ref.get(n, "")).lower() if n in ref else "",
            feat[n],
            note(n, c, dims),
        ]
    )
